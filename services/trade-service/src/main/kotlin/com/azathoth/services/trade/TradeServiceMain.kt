...
fun main() {
    logger.info { "正在启动 Trade Service..." }

    val marketService = DefaultMarketService()
    logger.info { "业务组件已初始化 (MarketService)" }
    ...
    logger.info { "Trade Service 启动完成" }

    Runtime.getRuntime().addShutdownHook(Thread {
        logger.info { "正在关闭 Trade Service..." }
        httpServer.stop(1000, 5000)
        grpcServer.shutdown()
        logger.info { "Trade Service 已关闭" }
    })
